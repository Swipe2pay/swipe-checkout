...
define('js/app',[
  ...
  var app = {
    init: function (config) {
      settings = config;
      var url = "https://backend.services.swipe2pay.co/api/v2/client/payment";

      var $style = $("<style></style>", { type: "text/css" });
      $style.text(css);
      $("head").append($style);
      $(formHtml).insertAfter("#swipe-checkout");
      // get a reference of the form after is inserted
      $form = $("#FORM-ID");
      var rootElement = $("#swipe-checkout");

      var key = rootElement.attr("data-swipe-api-key");
      var logo_src = rootElement.attr("data-swipe-logo");
      var order_id = rootElement.attr("data-swipe-payment-id");

      $("#amountInput").on("input", function () {
        $(".amount_error").addClass("error").removeClass("error-show");
      ...
      var env = rootElement.attr("data-swipe-env");
      if (env === "dev") {
        url = "https://backend.staging.swipe2pay.co/api/v2/client/payment";
      }
  
      var logo_background_color = rootElement.attr("data-swipe-logo-bgcolor");
      if (logo_background_color) {
        $(".modal__circle").css("background-color", logo_background_color);
      }
      var currency = "UGX";
      var swipeCurrency = rootElement.attr("data-swipe-currency");
      if (swipeCurrency) {
        currency = swipeCurrency;
      }

      var checkoutAmount = rootElement.attr("data-swipe-amount");
      var custPhone = rootElement.attr("data-swipe-phone");

      $("#swipe-checkout").change(function () {
        checkoutAmount = rootElement.attr("data-swipe-amount");
        custPhone = rootElement.attr("data-swipe-phone");
        key = rootElement.attr("data-swipe-api-key");
      });

      if (!checkoutAmount) {
        $("#amount").text(formatCurrency(currency, "0"));
      } else {
        $("#amount").text(formatCurrency(currency, checkoutAmount));
      }

      if (checkoutAmount === "0" || !checkoutAmount) {
        $(".cartAmount").remove();
      } else {
        $(".userAmount").remove();
        // $("#amount").text(formatCurrency(currency, checkoutAmount));
      ...
      $("#paymentButton").click(function (event) {
        ...
        var unformattedAmount = $("#amount").text();
        var amount = "";
        if (checkoutAmount === "0" || !checkoutAmount) {
          var newAmount = $("#amountInput").val();
          if (newAmount === "") {
            $(".amount_error").removeClass("error").addClass("error-show");
            $("#amountInput").addClass("input-error");
          } else {
            amount = newAmount;
          }
        } else {
          amount = unFormatCurrency(unformattedAmount);
        }
       
        if (amount === "") {
        } else {
          $("#paymentButton").html("Initiating ...");
          $("#paymentButton").addClass("spinning");
          ...
          dataType: "json",
          data: { phone: phone, amount, item_name: "Custom Item", order_id },
          success: function (response) {
            $("#paymentButton").removeClass("spinning");
            $("#paymentButton").html("PAY NOW");
            $(".modal__content").html(`
                <div class="payment-success">
                    ...
                    <div class="body">
                        ...
                        <p>Your transaction, was initiated,</br>
                          please wait 30 seconds,</br>
                          to enter your PIN </br>
                          on your phone to approve</p>
  ...
  function unFormatCurrency(amount) {
    var currencyStr = amount.slice(4);
    var separator = new Intl.NumberFormat().format(1111).replace(/1/g, "");
    return currencyStr.replace(new RegExp("\\" + separator, "g"), "");
  }
